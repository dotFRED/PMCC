...
const float TAFS_Factor = .8;  // TimeAllowedForShot Factor (TAFS * TAFS_Factor = new TAFS)
...
randomSeed(analogRead(A0));    //  initialize the pseudo random number generator by doing analogRead() on pin 0
...
*/
    ...
    do
    {
    resetSwitchVal = digitalRead(resetSwitchPin);  // read reset switch and handle accordingly
...
Serial.println(TTA);
    ...
    while (elapsedTotalTime < TTA )
    {
        do
        {
        randomTarget = random(0,number_of_targets);  //  Select a target index between 0 and 4 - max value of function is not inclusive
        }
    ...
    //    delay(5000);
        ...
        while (elapsedTargetTime < TAFS & elapsedTotalTime < TTA)
        {
